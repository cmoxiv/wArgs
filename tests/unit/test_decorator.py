...
class TestWargsDecoratorParsing:
    ...
    def test_parse_required_string(self) -> None:
        ...
        # Flag is --greet-name but dest is the original param name
        args = greet.parse_args(["--greet-name", "World"])
    ...
    def test_parse_integer(self) -> None:
        ...
        args = repeat.parse_args(["--repeat-count", "5"])
        assert args.count == 5
    ...
    def test_parse_boolean_flag(self) -> None:
        ...
        args_on = process.parse_args(["--process-verbose"])
    ...
    def test_parse_list(self) -> None:
        ...
        args = process.parse_args(["--process-files", "a.txt", "b.txt"])
    ...
    def test_parse_choices_literal(self) -> None:
        ...
        args = export.parse_args(["--export-format", "json"])
...
class TestWargsDecoratorExecution:
    ...
    def test_run_method(self) -> None:
        ...
        result = add.run(["--add-a", "2", "--add-b", "3"])
        assert result == 5
...
class TestWargsDecoratorEnums:
    ...
    def test_enum_choices(self) -> None:
        ...
        args = paint.parse_args(["--paint-color", "RED"])
...
class TestWargsDecoratorIntegration:
    ...
    def test_complete_cli_workflow(self) -> None:
        ...
        # Test parsing - namespace uses original param names as dest
        args = greet.parse_args(
            [
                "--greet-name",
                "World",
...
class TestWargsClassDecorator:
    ...
    def test_global_options_from_init(self) -> None:
        ...
        help_text = CLI.parser.format_help()
        assert "--CLI-verbose" in help_text
    ...
    def test_subcommand_execution(self) -> None:
        ...
        result = CLI.run(["add", "--add-name", "foo"])
    ...
    def test_global_options_passed_to_instance(self) -> None:
        ...
        result = CLI.run(["--CLI-verbose", "run"])
    ...
    def test_direct_instantiation(self, monkeypatch) -> None:
        ...
        # Explicit kwarg count=42 should override CLI's --Config-count 99
        instance = Config(count=42)
...
class TestWargsClassInheritance:
    ...
    def test_inherits_parent_options(self) -> None:
        ...
        help_text = CLI.parser.format_help()
        assert "--CLI-name" in help_text
        assert "--CLI-debug" in help_text
    ...
    def test_inherited_options_work_with_kwargs(self) -> None:
        ...
        result = CLI.run(["--CLI-debug", "--CLI-name", "test", "show"])
    ...
    def test_mixin_options_displayed(self) -> None:
        ...
        assert "--CLI-name" in help_text
        assert "--CLI-verbose" in help_text
        assert "--CLI-dry-run" in help_text
    ...
    def test_mixin_options_work_with_cooperative_inheritance(self) -> None:
        ...
        result = CLI.run(["--CLI-verbose", "--CLI-name", "test", "show"])
    ...
    def test_disable_mro_traversal(self) -> None:
        ...
        assert "--CLI-debug" not in help_text
...
class TestWargsDecoratorCoverageGaps:
    ...
    def test_parameter_without_annotation(self) -> None:
        ...
        result = process.run(["--process-data", "test"])
        assert result == "test"
    ...
    def test_subcommand_method_kwargs_empty_subconfig(self) -> None:
        ...
        namespace = CLI.parse_args(["add", "--add-name", "test"])
    ...
    def test_class_call_without_args_parses_cli(self, monkeypatch) -> None:
        ...
        # Patch sys.argv with a CLI argument
        monkeypatch.setattr("sys.argv", ["test", "--Config-name", "from_cli"])

        # Calling without args should parse CLI and create instance
        instance = Config()
        assert isinstance(instance, Config._cls)
        assert instance.name == "from_cli"  # CLI value applied
    ...
    def test_var_positional_skipped(self) -> None:
        ...
        result = process.run(["--process-count", "5"])
        assert result["count"] == 5
        assert result["items"] == ()
    ...
    def test_var_keyword_skipped(self) -> None:
        ...
        result = process.run(["--process-name", "test"])
        assert result["name"] == "test"
